...
class Catalog extends DataManager {
    ...
    public function getResult($function_name, $arguments, $is_array = false, $get_amount = false) {
        if (!$this->soap)
            return false;

        $result = $this->soap->call($function_name, $arguments, $is_array);
        $arResult = $this->checkElement($result, $get_amount);
        return $arResult;
    }
    ...
    public static function checkElement($result, $get_amount = false) {
        if(!$get_amount){
            ...
            $arResult = array();
            if (array_key_exists(static::NAME, $result) && array_key_exists(0, $result[static::NAME])){
                $arResult[static::NAME] = $result[static::NAME];
                $arResult[static::AMOUNT] = $result[static::AMOUNT];
                return $arResult;
            }
            ...
            elseif (array_key_exists(static::GROUPS, $result)){
                if(array_key_exists(static::ONE_ELEMENT, $result[static::GROUPS])){
                    $arOneElement[] = $result[static::GROUPS];
                    $arResult[static::NAME] = $arOneElement;
                    $arResult[static::AMOUNT] = $result[static::AMOUNT];
                    return $arResult;
                }
                else{
                    $arResult[static::NAME] = $result[static::GROUPS];
                    $arResult[static::AMOUNT] = $result[static::AMOUNT];
                    return $arResult;
                }
            ...
            elseif (array_key_exists(static::ONE_ELEMENT, $result)) {
                ...
                $arResult[static::NAME]['0'] = $result[static::NAME];
                $arResult[static::AMOUNT] = $result[static::AMOUNT];
                return $arResult;
        }
