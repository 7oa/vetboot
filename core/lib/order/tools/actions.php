...
$arBacket = Basket::getItems($USER_ID);
...
switch ($type) {
    case 'chek_date':
        $start_add_basket = strtotime($arBacket[0]['TIMESTAMP_X']);
        $today = strtotime(date("Y-m-d H:i:s"));
        $today_kontrol = strtotime(date("Y-m-d").' 10:30:00');
        $past_time = $today - $start_add_basket;
        $minutes = floor($past_time/60);
        $ar_date['date'] = $arBacket[0]['TIMESTAMP_X'];
...
//        if($start_add_basket < $today_kontrol){
//            if($today > $today_kontrol){
//                $ar_date['res'] = '1';
//            }else{
//                $ar_date['res'] = '0';
//            }
//        }
        echo json_encode($ar_date);
        break;
    case 'add':
        ...
        foreach ($backet as $arProducts) {
            $products[] = array(
                ...
                'quantity' => (float) $arProducts["QUANTITY"],
        ...
        //echo "<pre style='display:none;'>"; echo json_encode($addFields); echo "</pre>";
